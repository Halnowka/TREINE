...
export function WorkoutDayToggle({ selectedDay, onSelectDay }: WorkoutDayToggleProps) {
  return (
    <div className="flex justify-center flex-wrap gap-2 md:space-x-4 mb-6">
      <Button
      ...
      <Button
        variant={selectedDay === 'russian' ? 'default' : 'outline'}
        onClick={() => onSelectDay('russian')}
        className={`px-6 py-3 text-lg rounded-lg transition-all duration-150 ease-in-out transform hover:scale-105 lowercase ${
          selectedDay === 'russian' ? 'bg-primary text-primary-foreground shadow-lg' : 'border-primary text-primary hover:bg-accent hover:text-accent-foreground'
        }`}
